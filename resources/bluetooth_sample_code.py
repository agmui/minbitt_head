...
class ifm_and_fm_bluetooh_functions:
    ...
    async def bluetooth_recieve_realtime_start(self, message):
        ...
        async def has_required_characteristics(c):
            ...
            print("try to get characteristics", svcs.characteristics)
            try:
                has_notify = svcs.get_characteristic(self.NOTIFY_CHARACTERISTIC_UUID) is not None
                print(svcs.get_characteristic(self.NOTIFY_CHARACTERISTIC_UUID) )
        ...
        try:
            # 既接続なら送るだけ
            # If already connected, simply send.
            ...
            # Priority Order: Last Successful Address → Name Match → Remaining Results (Max. 15)
            ...
            for i, dev in enumerate(priority):
                ...
                c = BleakClient(dev, timeout=(12.0 if i == 0 else 10.0))  # 1台目だけ少し余裕 Only the first unit has a little bit of wiggle room.
                try:
                    await c.connect()

                    # ★ ここが肝：サービス反映待ち＋総当たりチェック
                    # The Key Point: Waiting for Service Updates + Exhaustive Checks
                    if False and not await has_required_characteristics(c):
            ...
            for s in client.services:
                print(s)
            face_track_service = client.services.services[57]
            # face_track_service = client.services.services[64]
            print(face_track_service)
            print()
            print("-- characteristics --")
            char_to_use = None
            for chars in face_track_service.characteristics:
                if chars.uuid == self.NOTIFY_CHARACTERISTIC_UUID.lower():
                    char_to_use = chars
                print(chars)
                print(chars.properties)
                for desc in chars.descriptors:
                    print(desc)
                print()
            if char_to_use == None:
                print("ERROR chars_to_use is None")
            print(char_to_use.uuid)
            # await client.start_notify(self.NOTIFY_CHARACTERISTIC_UUID, handle_notification)
            await client.start_notify(char_to_use, handle_notification)
            # await client.start_notify(71, handle_notification)
            print("after start_notify()")
        ...
        if self.bluetooth_client and self.bluetooth_client.is_connected:
            try:
                await self.bluetooth_client.write_gatt_char(
                     65,# self.WRITE_CHARACTERISTIC_UUID.lower(),
...
if __name__ == "__main__":
    ...
    # bluetooth_instance.bluetooth_send_message_trigger("FACEMOTION3D_OtherStreaming")
